...
RUN_SECONDS = int(os.environ.get("RUN_SECONDS", "20400"))  # ek run itni der (~5.6h) position dekhta rahega
MAX_ERRORS_IN_A_ROW = 8       # itni baar lagatar glitch aaye to hi session rokein (warna retry karta rahe)
...
def manage_positions(client, positions, filters):
    for sym, pos in positions.items():
        try:
            _manage_one_position(client, sym, pos, filters)
        except Exception as e:
            # Ek symbol ka glitch (network/API) poori monitoring session crash
            # nahi karega - SL/TP exchange par pehle se lagi hui hai, agla
            # cycle (WATCH_EVERY sec baad) dobara try karega.
            log(f"{sym}: monitor error ({type(e).__name__}: {e}) - agla cycle retry karega")


def _manage_one_position(client, sym, pos, filters):
...
def run_session(client):
    ...
    while True:
        try:
            ...
            # Koi glitch (network/API/rate-limit) - crash nahi hona, kyunki
            # agar position khuli hai to usay monitor karna zaroori hai.
            errors_in_a_row += 1
            log(f"cycle error ({type(e).__name__}: {e}) [{errors_in_a_row}/{MAX_ERRORS_IN_A_ROW}]")
            if errors_in_a_row >= MAX_ERRORS_IN_A_ROW:
                log("Lagatar bohat saari errors - is run ko rokte hain, agla cron run dobara try karega")
                return
            time.sleep(min(WATCH_EVERY * errors_in_a_row, 120))  # backoff, phir retry
            continue
        first = False
        if not has_pos:
            return                      # khuli position nahi - agla scan agle run/cron me
